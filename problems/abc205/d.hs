...
main = do
  [_, q] <- get @[Int]
  xs <- V.thaw =<< get @(V.Vector Integer)
  qs <- getLn @Integer @V.Vector q
  V.forM_ qs \k -> do
    let
      over :: Integer -> IO Bool
      over x = do
        y <- VAS.binarySearchR xs x
        return $ x - fromIntegral y >= k
    ans <- binarySearchM -1 (10 ^ (18 :: Int) + 10 ^ (5 :: Int) + 1) over
    print ans

binarySearchM :: (Monad m , Integral t) => t -> t -> (t -> m Bool) -> m t
binarySearchM l r f
  | abs (r - l) <= 1 = return r
  | otherwise  = do
    let
    ...
    res <- f mid
    if res
      then binarySearchM l mid f
      else binarySearchM mid r f
