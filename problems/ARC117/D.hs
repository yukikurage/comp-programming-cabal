...
main = do
  n <- get @Int
  xs <- getLn @(Int, Int) (n - 1)
  let graph = V.create do
        v <- VM.replicate n []
        VU.forM_ xs \(i, j) -> VM.modify v ((j - 1) :) (i - 1) >> VM.modify v ((i - 1) :) (j - 1)
        return v
      ys = solve n graph
  putStrLn $ vecToStr ys
  return ()

solve :: Int -> V.Vector [Int] -> VU.Vector Int
solve n graph = runST do
  ...
  let loop now = do
        ...
        VUM.write v now m
        modifySTRef' ref (+ 1)
        forM_ (graph ! now) \i -> do
          x <- VUM.read v i
          when (x == 0) (loop i >> modifySTRef' ref (+ 1))
      mono = fromJust . V.find (\i -> length (graph ! i) == 1) $ [0 .. n - 1]
  loop mono
  VU.freeze v
