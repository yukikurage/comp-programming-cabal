...
import qualified Data.Foldable                     as Folda
...
main = do
  ...
  t <- VUM.replicate (10 ^ 6 + 2) ' '
  isRev <- IORef.newIORef False
  l <- IORef.newIORef (5 * 10 ^ 5 :: Int)
  r <- IORef.newIORef (5 * 10 ^ 5 :: Int)
  VU.forM_ s \c -> if c == 'R' then IORef.modifyIORef' isRev not else do
    frag <- IORef.readIORef isRev
    if frag
    then do
      left <- IORef.readIORef l
      d <- VUM.read t left
      if c == d
        then IORef.writeIORef l $ left + 1
        else IORef.writeIORef l (left - 1) >> VUM.write t (left - 1) c
    else do
      right <- IORef.readIORef r
      d <- VUM.read t (right - 1)
      if c == d
        then IORef.writeIORef r $ right - 1
        else IORef.writeIORef r (right + 1) >> VUM.write t right c
  frag <- IORef.readIORef isRev
  left <- IORef.readIORef l
  right <- IORef.readIORef r
  Prelude.putStrLn . VU.toList . (if frag then VU.reverse else id) . VU.slice left (right - left) =<< VU.freeze t
