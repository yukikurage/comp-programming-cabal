...
main = do
  _ <- get @Int
  xs <- get @(V.Vector (Inf Int))
  ys <- get @(V.Vector (Inf Int))
  let
    sunuke1 = min (V.head ys) $ V.foldl' (\acc (x, y) -> min acc y + x) infinity $ V.zip xs ys
    answer = V.scanl' (\acc (x, y) -> min (acc + x) y) sunuke1 $ V.zip xs $ V.tail ys
  V.forM_ answer print
