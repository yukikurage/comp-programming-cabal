...
facts :: V.Vector Integer
facts = V.scanl' (*) 1 [1 .. 60]

choose n r = facts ! n `div` ((facts ! (n - r)) * (facts ! r))

main :: IO ()
main = do
  [a, b, k] <- get @[Int]
  print . BS.pack $ solve a b $ toInteger k
  return ()

solve a b k
  | a == 0 = replicate b 'b'
  | b == 0 = replicate a 'a'
  | k <= choose (a + b - 1) b = 'a' : solve (a - 1) b k
  | k > choose (a + b - 1) b = 'b' : solve a (b - 1) (k - choose (a + b - 1) b)
