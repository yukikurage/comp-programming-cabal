...
main = do
  n <- get @Int
  V.mapM_ putStrLn . V.reverse $ solve n 0 'a'
  return ()

solve 1 _ c = [[c]]
solve n m c = V.map (c :) . V.concat $ solve (n - 1) (m + 1) (Char.chr (Char.ord 'a' + m + 1)) : map (solve (n - 1) m) [Char.chr (Char.ord 'a' + m), Char.chr (Char.ord 'a' + m - 1) .. 'a']
