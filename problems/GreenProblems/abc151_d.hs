...
main = do
  [h, w] <- get @[Int]
  xs' <- V.replicateM h (VU.fromList <$> getLine)
  let
    xs = fmap f . A.listArray ((0, 0), (h - 1, w - 1)) $ A.range ((0, 0), (h - 1, w - 1)) :: AU.Array (Int, Int) Char where
      f (i, j) = xs' ! i ! j
  print . maximum . map (solve h w xs) $ A.range ((0, 0), (h - 1, w - 1))
  return ()

solve :: Int -> Int -> AU.Array (Int, Int) Char -> (Int, Int) -> InfInt
solve h w xs (i, j) = if xs A.! (i, j) == '#' then 0 else maximum $ filter (/= infinity) $ A.elems res where
  res :: A.Array (Int, Int) InfInt
  res = AST.runSTArray do
    a <- AST.newArray ((0, 0), (h - 1, w - 1)) infinity
    AST.writeArray a (i,j) 0
    let
      outBounds (x, y) = x < 0 || x >= h || y < 0 || y >= w
      loop (Seq.viewl -> Seq.EmptyL)   = return ()
      loop (Seq.viewl -> pos@(x, y) Seq.:< q) = do
        dist <- AST.readArray a pos
        let
          f pos'
            | outBounds pos' || xs A.! pos' == '#' = return False
            | otherwise = do
              dist' <- AST.readArray a pos'
              if dist' == infinity
              then do
                AST.writeArray a pos' $ dist + 1
                return True
              else return False
        loop . addSeq q =<< M.filterM f [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]
    loop $ Seq.singleton (i, j)
    return a

addSeq s []     = s
addSeq s (x:xs) = addSeq (s Seq.|> x) xs
