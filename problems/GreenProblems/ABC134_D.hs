...
main = do
  ...
  let
    ...
    calc i
      | i == 0 = False
      | i > n `div` 2 = xs ! (i - 1) == 1
      | otherwise = odd . (+ xs ! (i - 1)) . VU.length . VU.filter (ys !)
        $ [2 * i, 3 * i  .. n]
    ans = V.filter (ys !) [1 .. n]
  print . V.length $ ans
  when (V.length ans /= 0) $ BS.putStrLn . showVec $ ans
...
(<<+) (Counter cntr) a = Counter next where
  next =
    if Map.member a cntr then Map.adjust (+1) a cntr else Map.insert a 1 cntr
