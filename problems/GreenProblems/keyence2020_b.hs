...
main = do
  n <- get @Int
  robots' <- getLn @(Int, Int) @VU.Vector n
  let
    robots = VU.modify (VAM.sortBy (\x y -> compare (fst x) (fst y))) . VU.map (\(pos, len) -> (pos - len, len * 2)) $ robots'
  print $ solve n robots ! 0
  return ()

solve n robots = VU.create do
  poses <- VU.thaw . VU.map fst $ robots
  dp <- VUM.new n
  VUM.write dp (n - 1) (1 :: Int)
  VU.forM_ [n - 2, n - 3 .. 0] \i -> do
    index <- VAS.binarySearchL poses ((fst . (!) robots $ i) + (snd . (!) robots $ i))
    m <- if index == n then return 0 else VUM.read dp index
    n <- VUM.read dp (i + 1)
    VUM.write dp i $ max (m + 1) n
  return $ dp
