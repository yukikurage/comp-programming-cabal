...
main = do
  [n, k] <- get @[Int]
  s <- BS.getLine
  let l = (if BS.head s == '1' then [-1, -1] else [-1]) VU.++ VU.filter (\i -> BS.index s i == '0' && BS.index s (i + 1) == '1') [0 .. n - 2]
      r = VU.filter (\i -> BS.index s i == '1' && BS.index s (i + 1) == '0') [0 .. n - 2] VU.++ (if BS.head s == '1' then [n - 1, n - 1] else [n - 1])
  print if VU.length l - (k - 1) <= 0 then n else VU.maximum . VU.zipWith (flip (-)) (VU.take (VU.length l - (k - 1)) l) $ VU.drop (k - 1) r
  return ()
