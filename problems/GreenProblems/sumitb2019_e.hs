...
main = do
  ...
  print (solve xs [] :: ModInt 1000000007)
  return ()

solve :: VU.Vector Int -> VU.Vector Int -> ModInt 1000000007
solve xs lasts
  | VU.null xs = if VU.length lasts == 1 then modint 3 else modint 6
  | x == 0 = if VU.length lasts < 3 then solve xs' (0 `VU.cons` lasts) else modint 0
  | otherwise = if n == 0 then zero else modint n * solve xs' lasts'
  where
  x = VU.head xs
  xs' = VU.tail xs
  bef = Maybe.fromJust . VU.findIndex (== x - 1) $ lasts
  n = VU.length . VU.filter (== x - 1) $ lasts
  lasts' = lasts VU.// [(bef, x)]
