...
primeNum :: Int -> Int
primeNum n = length . Set.elems . Set.delete 1 $ loop n where
  loop m = case res of
    Just k  -> Set.insert k $ loop (m `div` k)
    Nothing -> Set.singleton m
    where
      res = VU.find (\k -> m `mod` k == 0) [2 .. floor . sqrt . fromIntegral $ n]

main :: IO ()
main = do
  [a, b] <- get @[Int]
  print . (+ 1) . primeNum $ gcd a b
  return ()
