...
main = do
  ...
  let res = solve xs n k
  print . VU.sum . VU.map (\i -> if res ! i == -1 then 0 else n - i - res ! i) $ [0 .. n - 1]
  return ()

solve xs n k = VU.create do
  v <- VUM.replicate n (-1)
  acc <- newSTRef $ xs ! 0
  let
    loop i j
      ...
      res <- readSTRef acc
      if res >= k then do
        modifySTRef' acc (+ (- xs ! i))
        VUM.write v i $ j - i
        loop (i + 1) j else do
        when (j <= n - 2) $ modifySTRef' acc (+ (xs ! (j + 1)))
        loop i (j + 1)
  loop 0 0
  return v
