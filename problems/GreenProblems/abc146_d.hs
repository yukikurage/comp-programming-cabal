...
main = do
  !n <- get @Int
  !edges' <- getLn @(Int, Int) @VU.Vector (n - 1)
  let
    edges = xs VU.++ VU.map (\(i,j,k) -> (j, i, k)) xs where
      xs = VU.map (\i -> (fst (edges' ! i) - 1, snd (edges' ! i) - 1, i)) [0 .. n - 2]
    g = gFromEdges n edges
    m = V.maxIndex . V.map length $ g
    k = length (g ! m)
    t = dfsWithLabel g m
  cols <- VUM.new (n - 1)
  let
    loop (Tree.Node nowLab []) nowCol = M.when (nowLab /= -1) $ VUM.write cols nowLab nowCol
    loop (Tree.Node nowLab nextTrees) nowCol = do
      let
        nextCols = V.take (length nextTrees) . V.filter (/= nowCol) $ [1 .. k]
      M.when (nowLab /= -1) $ VUM.write cols nowLab nowCol
      V.zipWithM_ loop (V.fromList nextTrees) nextCols
  loop t -1
  print k
  VU.freeze cols >>= VU.mapM_ print
  return ()

dfsWithLabel :: Graph Int -> Int -> Tree.Tree Int
dfsWithLabel g i = ST.runST do
  reached <- VUM.replicate (V.length g) False
  let
    loop (now, lab) = do
      VUM.write reached now True
      !nexts <- M.filterM (\(next, _) -> not <$> VUM.read reached next) $ g ! now
      !childs <- M.mapM loop nexts
      return $ Tree.Node lab childs
  loop (i, -1)

data VTree a = Node a (V.Vector (VTree a)) deriving Eq
