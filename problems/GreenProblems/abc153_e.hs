...
main = do
  ...
  let
    dp = solve xs n h
    ans = fromInf $ dp A.! (A.Z A.:. n A.:. h)
  print $ Maybe.fromMaybe (error "Inf Err") ans
  return ()

solve :: VU.Vector (Int, Int) -> Int -> Int -> A.Array ARV.V A.DIM2 (Inf Int)
solve xs n h = dp where
  dp = A.computeS . A.fromFunction (A.Z A.:. n + 1 A.:. h + 1) $ f
  f (_ A.:. _ A.:. 0) = 0
  f (_ A.:. 0 A.:. _) = infinity
  f (_ A.:. i A.:. j) = min cand1 . min cand2 $ cand3 where
    (a, b) = xs ! (i - 1)
    cand1 = dp A.! (A.Z A.:. i - 1 A.:. j)
    bef = max 0 $ j - a
    cand2 = (dp A.! (A.Z A.:. i - 1 A.:. bef)) + fromIntegral b
    cand3 = (dp A.! (A.Z A.:. i A.:. bef)) + fromIntegral b
...
main = do
  ...
  let
    dp = solve xs n h
    ans = dp A.! (A.Z A.:. n A.:. h)
  print $ ans
  return ()

solve :: VU.Vector (Int, Int) -> Int -> Int -> A.Array A.U A.DIM2 Int
solve xs n h = A.computeS  dp where
  dp =A.fromFunction (A.Z A.:. n + 1 A.:. h + 1) $ f
  f (_ A.:. _ A.:. 0) = 0
  f (_ A.:. 0 A.:. _) = 10^10
  f (_ A.:. i A.:. j) = min cand1 . min cand2 $ cand3 where
    (a, b) = xs ! (i - 1)
