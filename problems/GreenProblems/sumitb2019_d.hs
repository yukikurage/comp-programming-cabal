...
main = do
  n <- get @Int
  s <- VU.map Char.digitToInt . VU.fromList <$> getLine
  let
    (_, _, res) = solve s
  print . VU.length . VU.filter id $ res
  return ()

solve :: VU.Vector Int -> (VU.Vector Bool, VU.Vector Bool, VU.Vector Bool)
solve s
  | VU.null s = (VU.replicate 10 False, VU.replicate 100 False, VU.replicate 1000 False)
  | otherwise = (xs', ys', zs')
  where
  (xs, ys, zs) = solve (VU.tail s)
  x = VU.head s
  xs' = VU.generate 10 (\i -> i == x || xs ! i)
  ys' = VU.generate 100 (\i -> (i `div` 10 == x && xs ! (i `mod` 10)) || ys ! i)
  zs' = VU.generate 1000 (\i -> (i `div` 100 == x && ys ! (i `mod` 100)) || zs ! i)
