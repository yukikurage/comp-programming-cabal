...
main = do
  ...
  let
    m = VU.maximum xs
    ys = VU.take (m + 1) $ VU.create do
      v <- VUM.replicate n (0 :: Int)
      VU.forM_ xs $ VUM.modify v (+1)
      return v
  if 0 `VU.elem` ys || ys ! 0 >= 2 || xs ! 0 /= 0
    then putStrLn "0"
    else print $ V.foldl' (*) (modint 1) . V.map (\i -> modint (ys ! i) ^ (ys ! (i + 1)) :: ModInt 998244353) $ [0 .. m - 1]
  return ()
