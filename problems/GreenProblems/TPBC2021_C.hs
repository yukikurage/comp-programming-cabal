...
main = do
  ...
  let black = VU.scanl' (\acc c -> if c == '#' then acc + 1 else acc) (0 :: Int) $ s
      white = VU.scanr' (\c acc -> if c == '.' then acc + 1 else acc) (0 :: Int) $ s
  print . VU.minimum . VU.map (\i -> white ! i + black ! i) $ [0 .. n]
  return ()
