...
main = do
  ...
  let
    ps = preSufs n
  print . A.sum . A.map (count ps) $ A.range A.Seq (1 :. 1) (10 :. 10)

preSufs :: Int -> A.Array A.U A.Ix2 Int
preSufs n = ST.runST do
  a <- AM.new (A.Sz2 10 10)
  VU.forM_ [1 .. n] \m -> do
    let
      s = showBS m
      pre = Char.digitToInt $ BS.head s
      suf = Char.digitToInt $ BS.last s
    AM.modifyM_ a (return . (+ 1)) (pre :. suf)
  AM.freezeS a

count :: A.Array A.U A.Ix2 Int -> A.Ix 2 -> Int
count ps (i :. j) = (ps A.! (j :. i)) * (ps A.! (i :. j))
