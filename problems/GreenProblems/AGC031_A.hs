...
main = do
  n <- get @Int
  s <- BS.getLine
  let m = A.listArray ('a', 'z') [0 .. 25] :: A.Array Char Int
      xs = VU.map (\i -> m A.! BS.index s i) [0 .. n - 1]
      ys = V.create do
        v <- VM.replicate 26 (modint 1 :: ModInt 1000000007)
        VU.forM_ xs $ VM.modify v (+ modint 1)
        return v
  print $ V.foldl1' (*) ys - modint 1
  return ()
