...
main = do
  ...
  let
    a = length [True | i <- [0 .. n - 2], j <- [i + 1 .. n - 1], xs ! i > xs ! j]
    ys = VU.modify VAM.sort xs
  dup <- VM.replicate 2001 Nothing
  b <- VU.sum <$> VU.generateM n (\i -> do
    let x = ys ! i
    frag <- VM.read dup x
    case frag of
      Nothing -> VM.write dup x (Just i) >> return i
      Just j  -> return j
    )
  print (modint k * (modint a + modint b * modint (k - 1) / modint 2):: ModInt 1000000007)
  return ()
