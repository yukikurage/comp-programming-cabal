...
main = do
  [h, w, k] <- get @[Int]
  cake <- V.replicateM h (V.fromList <$> getLine)
  let
    divs = mkdivs h w cake
  V.forM_ divs print

mkdivs :: Int -> Int -> V.Vector (V.Vector Char) -> V.Vector (V.Vector Int)
mkdivs h w cake = case fstBerry of
  Just pos -> divs where
    divs = V.map f [0 .. h - 1]
    f i
      | i < pos = divs ! (i + 1)
      | i == pos = mkyoko w (cake ! i) 1
      | V.any (== '#') (cake ! i) = mkyoko w (cake ! i) $ divs ! (i - 1) ! (w - 1) + 1
      | otherwise = divs ! (i - 1)
  Nothing -> V.replicate h $ V.replicate w 1
  where
  fstBerry = V.findIndex (V.any (== '#')) cake

mkyoko :: Int -> V.Vector Char -> Int -> V.Vector Int
mkyoko w cake index = case fstBerry of
  Just pos -> divs where
    divs = V.map f [0 .. w - 1]
    f i
      | i < pos = divs ! (i + 1)
...
instance ReadBS Char where
  readBS s
    | BS.length s == 1 = BS.head s
    | otherwise = error "Invalid Format :: readBS :: ByteString -> Char"
