...
main = do
  n <- get @Int
  xs <- V.indexed . V.map (modint @200) <$> get @(V.Vector Int)
  case solve xs (V.replicate 200 Nothing) of
    Nothing -> putStrLn "No"
    Just [xs', ys] -> do
      let
        x = length xs'
      ...
      printF x
      putStr " "
      print . map (+ 1) . reverse $ xs'
      printF y
      putStr " "
      print . map (+ 1) . reverse $ ys
  return ()

solve :: V.Vector (Int, ModInt 200) -> V.Vector (Maybe [Int]) -> Maybe [[Int]]
solve xs seqs
  | V.null xs = Nothing
  | otherwise = case res of
  Nothing   -> solve xs' seqs''
  Just res' -> Just res'
  where
  (j, x) = V.head xs
  xs' = V.tail xs
  res = V.find (\ys -> length ys >= 2) . V.map (\i -> take 2 . Maybe.catMaybes $ [seqs ! i, seqs' ! i, if modint i == x then Just [j] else Nothing]) $ [0 .. 199]
  seqs' = V.map g [0 .. 199]
  g i
    | Maybe.isJust (seqs !: (modint i - x)) = (j : ) <$> (seqs !: (modint i - x))
    | otherwise = Nothing
  seqs'' = V.map f [0 .. 199]
  f i
    | Maybe.isJust (seqs ! i) = seqs ! i
    | modint i == x = Just [j]
    | otherwise = seqs' ! i

v !: i = v ! (fromIntegral . frommodint $ i)
