...
main = do
  [k, n, m] <- get @[Int]
  xs <- get @(VU.Vector Int) -- length xs == k
  let
    xs' = VU.scanl' (+) 0 xs
    ys' = VU.scanl' (+) 0 ys
    ys = VU.map (\x -> fromIntegral (x * m) / fromIntegral n) xs :: VU.Vector Double
    bests = solve n m xs ys (0, 0) [(True, True)]
    ans = VU.map (\i -> (if bests ! (i + 1) then floor else ceiling) (ys' ! (i + 1)) - (if bests ! i then floor else ceiling) (ys' ! i)) [0 .. k - 1] :: VU.Vector Int
  print ans
  return ()

gap :: (Fractional b, Integral a) => a -> a -> a -> a -> b
gap m n b a = abs $ fromIntegral b / fromIntegral m - fromIntegral a / fromIntegral n

solve n m xs ys (floorMax, ceilMax) bests
  | VU.length xs == 1 = VU.reverse . VU.scanl' (\frag (isFloor, isCeil) -> if frag then isFloor else isCeil) False $ b where
    b = VU.fromList bests
solve n m xs {- <-累積和 -} ys {- <-累積和 -} (floorMax, ceilMax) bests = solve n m xs' ys' (floorMax', ceilMax') (best : bests) where
  gap' = gap m n
  x = xs ! 0
  x' = xs ! 1
  y = ys ! 0
  ...
  xs' = VU.tail xs
  -- y'を前に寄せたとき
  floorIsBest1 = max floorMax (gap' (floor y' - floor y) (x' - x)) <= max ceilMax (gap' (floor y' - ceiling y) (x' - x))
  -- y'を後ろに寄せたとき
  floorIsBest2 = max floorMax (gap' (ceiling y' - floor y) (x' - x)) <= max ceilMax (gap' (ceiling y' - ceiling y) (x' - x))
  best = (floorIsBest1, floorIsBest2)
  floorMax' = min (max floorMax (gap' (floor y' - floor y) (x' - x))) (max ceilMax (gap' (floor y' - ceiling y) (x' - x)))
  ceilMax' = min (max floorMax (gap' (floor y' - floor y) (x' - x))) (max ceilMax (gap' (floor y' - ceiling y) (x' - x)))
