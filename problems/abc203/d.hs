...
main = do
  [n, k] <- get @[Int]
  xs <- V.replicateM n (get @(VU.Vector Int))
  print $ binarySearch 0 (10 ^ 9) (solve xs n k)
  return ()

solve xs n k cand = any f $ A.range ((0, 0), (n - k, n - k))
  where
  ys = V.map (VU.map g) xs
  g x = if x <= cand then 1 else 0
  zs = AST.runSTUArray do
    arr <- AST.newArray ((0, 0), (n, n)) 0
    let
      h (0, _)= return ()
      h (_, 0) = return ()
      h (i, j) = do
        up <- AST.readArray arr (i - 1, j)
        upleft <- AST.readArray arr (i - 1, j - 1)
        left <- AST.readArray arr (i, j - 1)
        AST.writeArray arr (i, j) $ up + left - upleft + ys ! (i - 1) ! (j - 1)
    M.forM_ (A.range ((0, 0), (n, n))) h
    return arr
  f (i, j) = zs A.! (i + k, j + k) - zs A.! (i, j + k) - zs A.! (i + k, j) + zs A.! (i, j) >= (k * k - k * k `div` 2)
