...
main = do
  ...
  print . V.length . V.filter (flt s) . V.map toDigit $ [0 .. 9999 :: Int]
  return ()

toDigit :: Int -> VU.Vector Int
toDigit n = VU.replicate (4 - len) 0 VU.++ dig
  where
  dig = VU.map Char.digitToInt . VU.fromList $ show n
  len = VU.length dig

flt :: VU.Vector Char -> VU.Vector Int -> Bool
flt s xs = VU.all (\i -> if
  | s ! i == 'o' -> VU.elem i xs
  | s ! i == 'x' -> VU.notElem i xs
  | otherwise    -> True) [0 .. 9]
