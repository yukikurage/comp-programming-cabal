...
main = do
  [h, w] <- get @[Int]
  xs <- A.listArray @AU.UArray ((0, 0), (h - 1, w - 1))  . concat <$> M.replicateM h getLine
  print $ solve h w xs
  return ()

solve h w xs
  | t < 0 = "Takahashi" :: BS.ByteString
  | t > 0 = "Aoki"
  | t == 0 = "Draw"
  where
  t = table h w xs A.! (0, 0)

table :: Int -> Int -> AU.UArray (Int, Int) Char -> AU.UArray (Int, Int) Int
table h w xs = AST.runSTUArray $ do
  dp <- AST.newArray_ ((0, 0), (h - 1, w - 1))
  let
    points (i, j) = if xs A.! (i, j) == '+' then 1 else -1
    f (i, j)
      | i == h - 1 && j == w - 1 = return 0
      | i == h - 1 && even (i + j) = AST.readArray dp (i, j + 1) >>= \x -> return $ x - points (i, j + 1)
      | i == h - 1 && odd (i + j) = AST.readArray dp (i, j + 1) >>= \x -> return $ x + points (i, j + 1)
      | j == w - 1 && even (i + j) = AST.readArray dp (i + 1, j) >>= \x -> return $ x - points (i + 1, j)
      | j == w - 1 && odd (i + j) = AST.readArray dp (i + 1, j) >>= \x -> return $ x + points (i + 1, j)
      | otherwise = do
        down <- AST.readArray dp (i + 1, j)
        right <- AST.readArray dp (i, j + 1)
        if even (i + j)
          then return $ min (down - points (i + 1, j)) (right - points (i, j + 1))
          else return $ max (down + points (i + 1, j)) (right + points (i, j + 1))
  sequence_ [f (i, j) >>= AST.writeArray dp (i, j) | i <- [h - 1 , h - 2 .. 0], j <- [w - 1, w - 2 .. 0]]
  return dp
