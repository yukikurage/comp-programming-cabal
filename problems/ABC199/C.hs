...
main = do
  ...
  s <- getLine
  q <- get @Int
  v <- VU.thaw $ VU.fromList s
  x <-newMutVar False
  let fs = FlipStr x n v
  replicateM_ q do
    (t, a, b) <- get @(Int, Int, Int)
    if t == 1 then swap fs (a - 1) (b - 1) else flip' fs
  putStrLn =<< flipshow fs

data FlipString m = FlipStr (MutVar m Bool) Int (VUM.MVector m Char)

swap :: PrimMonad m => FlipString (PrimState m) -> Int -> Int -> m ()
swap (FlipStr frag n bs) i j = do
  x <- readMutVar frag
  let i' = if x then (i + n) `mod` (2 * n) else i
  let j' = if x then (j + n) `mod` (2 * n) else j
  VUM.swap bs i' j'

flip' :: PrimMonad m => FlipString (PrimState m) -> m ()
flip' (FlipStr frag _ _) = do
  x <- readMutVar frag
  writeMutVar frag $ not x

flipshow :: forall m. PrimMonad m => FlipString (PrimState m) -> m String
flipshow (FlipStr frag n bs) = do
    (bef, aft) <- VU.splitAt n <$> VU.freeze bs :: m (VU.Vector Char, VU.Vector Char)
    x <- readMutVar frag
    let xs = if x then aft VU.++ bef else bef VU.++ aft
        res = VU.foldr' (:) "" xs
    return res
