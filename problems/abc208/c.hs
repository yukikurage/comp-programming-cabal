...
main = do
  ...
  let
    minDist = k `div` n
  sorting <- VU.thaw [0 .. n - 1]
  VAM.sortBy (\x y -> compare (xs ! x) (xs ! y)) sorting
  sorted <- VU.freeze sorting
  v <- VUM.replicate n minDist
  VU.forM_ (VU.take (k `mod` n) sorted) $ VUM.modify v (+1)
  ans <- VU.freeze v
  VU.forM_ ans print
