...
main = do
  [n, m] <- get @[Int]
  edges <- getLn @VU.Vector @(Int, Int, Int) m
  let g = gFromEdges n $ VU.map (\(i, j, w) -> (i - 1, j - 1, w)) edges
  print $ ans g
  return ()

ans :: Graph Int-> Int
ans g = VU.sum $ VU.fromList [f' g s k | k <- [1 .. n], s <- [0 .. n - 1]] where n = V.length g

f' :: Graph Int -> Int -> Int -> Int
f' g s k = V.sum $ V.map (Maybe.fromMaybe 0 . fromInf) $ limDijkstra g s k

-- | 使える頂点を制限したダイクストラ
limDijkstra :: Graph Int -> Int -> Int -> V.Vector (Inf Int)
limDijkstra g i limit = ST.runST do
  let
    n = V.length g
  d <- VM.new n
  V.forM_ [0 .. n - 1] \j -> let dj = if i == j then 0 else infinity in
    VM.write d j dj
  let
    q = PSQ.insert i 0 i $ PSQ.fromList [(j, infinity, j) | j <- [0 .. limit - 1]]
  loop d q
  V.freeze d
  where
    ...
    Just (_, _, u , q') -> do
      ...
      insertList <- Maybe.catMaybes <$> M.forM (g ! u) \(v, len) ->do
        ...
        if dist_v > alt
          then do
            VM.write d v alt
            if v >= limit then return Nothing else return (Just (v, alt, v))
          else return Nothing
      let
