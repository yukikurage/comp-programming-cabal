...
main = do
  ...
  let
    ys = timelist xs
    allTime = VU.sum xs
    midTime = (allTime + 1) `div` 2
    zs = VU.drop midTime ys
    minTime = VU.findIndex id zs
    ans = Maybe.fromJust $ (midTime +) <$> minTime
  print ans
  return ()

timelist :: VU.Vector Int -> VU.Vector Bool
timelist xs
  | VU.null xs = VU.replicate (10^(5 :: Int) + 1) False
  | otherwise = VU.generate (10^(5 :: Int) + 1) \i -> x == i || tl' ! i || (((i - x) >= 0) && (tl' ! (i - x)))
  where
  xs' = VU.tail xs
  x = VU.head xs
  tl' = timelist xs'
