...
main = do
  [n, m] <- get @[Int]
  edges <- getLn @(Int, Int, Int, Int) @VU.Vector m
  let
    g = gFromEdges n $ VU.concatMap(\(i, j, c, d) -> [(i - 1, j - 1, (c, d)), (j - 1, i - 1, (c, d))]) edges
    res = dijkstraS g 0
  print $ case fromInf $ res ! (n - 1) of
    Nothing -> -1
    Just x  -> x
  return ()


dijkstraS :: Graph (Int, Int) -> Int -> V.Vector (Inf Int)
dijkstraS g i = ST.runST do
  let
    n = V.length g
  d <- VM.new n
  q <- mpqNew
  ...
  while $ mpqMinView q >>= \case
    ...
    Just u -> do
      dist_u <- VM.read d u
      M.forM_ (g ! u) \(v, (ci, di)) -> do
        dist_v <- VM.read d v
        let
          alt = case fromInf dist_u of
            Nothing -> infinity
            Just ts -> fromIntegral $ ts + ci + (di `div` (ts + n' + 1)) + n' where
              n' = minN ts di
...
minN :: Integral a => a -> a -> a
minN ts d = res' where
  f n = (d `div` (ts + n + 1)) - (d `div` (ts + n + 2)) <= 1
  res = binarySearch (-1) (10^9) f
  resmin = max 0 $ res - 100
  res' = Maybe.fromJust $ V.find f [resmin, resmin + 1 .. res]

--ある場所から右がTrue
binarySearch :: Integral t => t -> t -> (t -> Bool) -> t
binarySearch l r f
  | r - l <= 1 = r
  | f mid = binarySearch l mid f
  | otherwise = binarySearch mid r f
  where
  mid = (l + r) `div` 2

{-
minN :: Integral a => a -> a -> a
minN ts d = max res ts
  where
  res = round (sqrt $ fromIntegral d) - 1
-}
-------------
-- Library --
-------------
